...
void ipush(int x) {			/* Insert the int in stack */
    if(sp > MAX) {
...
int isEmpty() {			/* Checking for stack empty */
    if(sp <= 0)
	return 1;
    return 0;
}
...
char getch() {			/* Simply uses getchar to take take input single cahracter at a time */
    char ch;
    if(bf <= 0) {
	ch = getchar();
    }
...
void stepwise() {
    int i;						/* Removing comments will print the postfix of given input */ 
    printf("-------------------------------\n");			
   /* for(i = 0; i < pt; i++)
